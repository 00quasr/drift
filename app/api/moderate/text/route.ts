...
import { checkRateLimit, getRateLimitHeaders, defaultRateLimit } from '@/lib/utils/rateLimit'
...
export async function POST(request: NextRequest) {
  try {
    // This endpoint spends money on the OpenAI API, so it is gated on a session
    ...
    const rateLimit = await checkRateLimit(`moderate-text:${user.id}`, defaultRateLimit)
