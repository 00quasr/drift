  ...
  const handleSocialLogin = async (provider: string) => {
    if (provider === 'google') {
      try {
        ...
        setError(error.message || 'Social registration failed. Please try again.')
      } finally {
        setIsLoading(false)
      }
    } else {
      console.log(`${provider} registration not implemented yet`)
    }
  }
      ...
      <div className="w-full max-w-lg">
        ...
        <Card className="bg-white/5 border border-white/20 p-8">
          ...
          <Button
            ...
            className="w-full h-12 border-2 border-white/30 bg-black text-white hover:bg-white/10 hover:border-cyan-400/60 transition-all font-bold tracking-wider uppercase"
          >
