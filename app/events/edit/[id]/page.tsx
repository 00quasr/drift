...
export default function EditEventPage({ params }: { params: { id: string } }) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setEvent({
          ...
          artists: eventData.artists?.map((artist: any) => ({
            id: artist.id,
            name: artist.name,
            performance_order: artist.event_artists?.performance_order || artist.performance_order,
            performance_type: artist.event_artists?.performance_type || artist.performance_type || 'headliner'
          })) || [],
          ticket_info: {
  ...
  const removeArtist = async (artistId: string) => {
    try {
      // Remove from API if event exists
      if (params.id) {
        const { supabase } = await import('@/lib/auth')
        const { data: { session } } = await supabase.auth.getSession()

        const response = await fetch(`/api/events/${params.id}/artists?artist_id=${artistId}`, {
          method: 'DELETE',
          headers: {
            ...(session?.access_token && {
              'Authorization': `Bearer ${session.access_token}`
        ...
        if (!response.ok) {
          throw new Error('Failed to remove artist from event')
        }
      }
  ...
  const handleSave = async (publishImmediately = false) => {
    ...
    try {
      ...
      if (response.ok) {
        // Save any new artists that were added to the lineup
        if (event.artists.length > 0) {
          console.log('Saving artist lineup changes for event:', params.id)
          
          // Get current artists from API to compare
          const currentArtistsResponse = await fetch(`/api/events/${params.id}/artists`, {
            headers: {
              ...(session?.access_token && {
                'Authorization': `Bearer ${session.access_token}`
              })
          ...
          if (currentArtistsResponse.ok) {
            const currentArtistsData = await currentArtistsResponse.json()
            currentArtists = currentArtistsData.data || []
          }
          
          const currentArtistIds = currentArtists.map(a => a.artist?.id || a.artist_id)
          
          // Add any new artists
          const newArtists = event.artists.filter(artist => !currentArtistIds.includes(artist.id))
          
          if (newArtists.length > 0) {
            const artistPromises = newArtists.map(async (artist) => {
              const artistResponse = await fetch(`/api/events/${params.id}/artists`, {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                  ...(session?.access_token && {
            ...
            console.log('New artists added to lineup successfully')
          }
        }
