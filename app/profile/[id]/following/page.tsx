...
export default function FollowingPage() {
  ...
  const fetchFollowing = async () => {
    try {
      ...
      const response = await fetch(`/api/users/${profileId}/following?page=${page}&limit=20`)
      ...
      if (page === 1) {
        ...
        setFollowing(prev => [...prev, ...data.data.following])
      }
  ...
  const getDisplayName = (profile: FollowingProfile) => {
            ...
            {following.map((followedUser) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <Link href={`/profile/${followedUser.following_id}`}>
                    <div className="w-12 h-12 bg-white/5 border border-white/20 flex items-center justify-center overflow-hidden">
                      {followedUser.profile.avatar_url ? (
                        <Image
                          src={followedUser.profile.avatar_url}
                          alt={getDisplayName(followedUser.profile)}
                          width={48}
                          height={48}
                  ...
                  <div className="flex-1 min-w-0">
                    <Link href={`/profile/${followedUser.following_id}`}>
                      <h3 className="text-white font-bold tracking-wider uppercase truncate hover:text-white/80 transition-colors">
                        {getDisplayName(followedUser.profile)}
                      </h3>
                    </Link>
                  ...
                  {user && user.id !== followedUser.following_id && (
                    <FollowButton
                      userId={followedUser.following_id}
                      initialIsFollowing={user.id === profileId}
                      size="sm"
                    />
                  )}
                </div>
