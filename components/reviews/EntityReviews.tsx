...
interface EntityReviewsProps {
  ...
  entityId: string
  entityName: string
  isPastEvent?: boolean // For events, only allow reviews on past events
}

export const EntityReviews: React.FC<EntityReviewsProps> = ({ 
  entityType, 
  entityId, 
  entityName,
  isPastEvent = true // Default true for non-events
}) => {
  const { user } = useAuth()
  ...
  // Get category labels based on entity type
  ...
  const categoryLabels = getCategoryLabels(entityType)

  const fetchReviews = async () => {
    try {
      const [reviewsResponse, statsResponse] = await Promise.all([
        fetch(`/api/reviews?target_type=${entityType}&target_id=${entityId}&sort=${sortBy}`),
        fetch(`/api/reviews/stats?target_type=${entityType}&target_id=${entityId}`)
      ])
      ...
      if (reviewsResponse.ok) {
        ...
        // Check if current user has already reviewed this entity
        if (user) {
  ...
  // For events, check if reviews are allowed (only past events)
  const canWriteReview = entityType !== 'event' || isPastEvent
    ...
    <>
      <div className="relative bg-black border-2 border-white/20 p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {user && canWriteReview && !userHasReviewed && (
            <button 
          ...
          {entityType === 'event' && !isPastEvent && (
            <div className="px-4 py-2 bg-white/5 text-white/60 border-2 border-white/20 font-bold tracking-wider uppercase text-sm">
              REVIEWS AFTER EVENT
            </div>
          )}
        </div>
                ...
                <div key={review.id} className="bg-white/5 border border-white/20 p-6">
                  ...
                  {(review.rating_sound || review.rating_vibe || review.rating_crowd) && (
                    <div className="grid grid-cols-3 gap-4 mb-4 text-center">
                      {review.rating_sound && (
                        <div>
                          <div className="text-white/60 font-bold tracking-widest uppercase text-xs mb-1">{categoryLabels.sound}</div>
                          <div className="text-white font-bold">{review.rating_sound}/10</div>
                        </div>
                      )}
                      {review.rating_vibe && (
                        <div>
                          <div className="text-white/60 font-bold tracking-widest uppercase text-xs mb-1">{categoryLabels.vibe}</div>
                          <div className="text-white font-bold">{review.rating_vibe}/10</div>
                        </div>
                      )}
                      {review.rating_crowd && (
                        <div>
                          <div className="text-white/60 font-bold tracking-widest uppercase text-xs mb-1">{categoryLabels.crowd}</div>
